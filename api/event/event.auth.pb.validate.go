...
// ValidateAll checks the field values on UserLogin with the rules defined in
// the proto definition for this message. If any rules are violated, the
// result is a list of violation errors wrapped in UserLoginMultiError, or nil
// if none found.
func (m *UserLogin) ValidateAll() error {
	return m.validate(true)
}

func (m *UserLogin) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	// no validation rules for IssueToken

	if all {
		switch v := interface{}(m.GetTimestamp()).(type) {
		case interface{ ValidateAll() error }:
			if err := v.ValidateAll(); err != nil {
				errors = append(errors, UserLoginValidationError{
					field:  "Timestamp",
	...
	if len(errors) > 0 {
		return UserLoginMultiError(errors)
	}

	return nil
}

// UserLoginMultiError is an error wrapping multiple validation errors returned
// by UserLogin.ValidateAll() if the designated constraints aren't met.
type UserLoginMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m UserLoginMultiError) Error() string {
...
func (m UserLoginMultiError) AllErrors() []error { return m }
